 ...
 */
export const getRoomById = async (id: number): Promise<ServiceResponse> => {
  try {
    const { data, error } = await supabase
      .from('rooms')
      .select('*')
    ...
    console.error(`Error al obtener habitación con ID ${id}:`, error.message);
    return {
      success: false,
      message: error.message
    };
 ...
 */
export const getRoomsByState = async (state: string): Promise<ServiceResponse> => {
  try {
    const { data, error } = await supabase
      .from('rooms')
      .select('*')
    ...
    console.error(`Error al obtener habitaciones con estado ${state}:`, error.message);
    return {
      success: false,
      message: error.message
    };
